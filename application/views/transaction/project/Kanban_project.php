                            ...
                            <div class="card card-danger">
                                ...
                                <div class="card-body sortable" data-status="STL-3">
                                    <?php if (!empty($ProjectList)) {
                                        foreach ($ProjectList as $row) : if (($row->status_id) == 'STL-3') :
                                                $tipeItem = ($row->priority_type_id == 'PR-3') ? 'success' : (($row->priority_type_id == 'PR-2') ? 'warning' : 'danger');
                                    ?>
                                                <div class="card card-<?= $tipeItem ?> card-outline " style="cursor: move; font-size:small;" data-id="<?= $row->project_id ?>">
                                                    <div class="card-header">
                                                        <div class="row">
                                                            <div class="col-md-8"><?= $row->project_name ?></div>
                                                            <div class="col-md-4 text-right p-0"><a class="btn btn-tool" href="<?= base_url() . 'ProjectItem/' . $ProjectWrkId . '/' . $row->project_id; ?>"><i class="fa fa-eye"></i></a></div>
                                                        </div>
                                                    </div>
                                                    <div class="card-body">
                                                        <?= $row->description ?>
                                                        ...
                                                        <div class="row" style="font-size: 12px;">
                                                            ...
                                                            <div class="col-6 text-right">
                                                                <?php foreach ($ProjectMemberRecords as $key) {
                                                                    if ($row->project_id == $key->project_id) : ?>
                        ...
                        <div class="col-lg-3">
                            <div class="card card-primary">
                                ...
                                <div class="card-body sortable" data-status="STL-1">
                                    <?php if (!empty($ProjectList)) {
                                        foreach ($ProjectList as $row) : if (($row->status_id) == 'STL-1') :
                                                $tipeItem = ($row->priority_type_id == 'PR-3') ? 'success' : (($row->priority_type_id == 'PR-2') ? 'warning' : 'danger');
                                    ?>
                                                <div class="card card-<?= $tipeItem ?> card-outline " style="cursor: move; font-size:small;" data-id="<?= $row->project_id ?>">
                                                    <div class="card-header">
                        ...
                        <div class="col-lg-3">
                            <div class="card card-warning">
                                ...
                                <div class="card-body sortable" data-status="STL-2">
                                    <?php if (!empty($ProjectList)) {
                                        foreach ($ProjectList as $row) : if (($row->status_id) == 'STL-2') :
                                                $tipeItem = ($row->priority_type_id == 'PR-3') ? 'success' : (($row->priority_type_id == 'PR-2') ? 'warning' : 'danger');
                                    ?>
                                                <div class="card card-<?= $tipeItem ?> card-outline " style="cursor: move; font-size:small;" data-id="<?= $row->project_id ?>">
                                                    <div class="card-header">
                        ...
                        <div class="col-lg-3">
                            <div class="card card-success">
                                ...
                                <div class="card-body sortable" data-status="STL-4">
                                    <?php if (!empty($ProjectList)) {
                                        foreach ($ProjectList as $row) : if (($row->status_id) == 'STL-4') :
                                                $tipeItem = ($row->priority_type_id == 'PR-3') ? 'success' : (($row->priority_type_id == 'PR-2') ? 'warning' : 'danger');
                                    ?>
                                                <div class="card card-<?= $tipeItem ?> card-outline " style="cursor: move; font-size:small;" data-id="<?= $row->project_id ?>">
                                                    <div class="card-header">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="card card-info card-outline" style="max-height: 300px;">
                                <div class="card-header">
                                    <input type="hidden" class="form-control" id="project_wrk_id" placeholder="Project ID" name="project_wrk_id" maxlength="20" value="<?= $ProjectWrkId; ?>" required readonly="true">
                                    <input type="hidden" class="form-control" id="stl_status" placeholder="Project ID" name="stl_status" maxlength="20" required readonly>
                                    <h5 class="card-title" style="width: 90%;"><input type="text" id="project_name" class="form-control" placeholder="Item Name"></h5>
                                    <div class="card-tools">
...
<script>
    ...
    $(document).on('click', '#AddProject', function() {
        var itemId = $('#project_wrk_id').val();
        var title = $('#project_name').val();
        var itemStart = $('#project_start').val();
        var itemDue = $('#project_due').val();
    ...
    function handleClickAddItem() {
        ...
        $('#members_project_item').select2({
            ...
            data: [
                <?php foreach ($ProjectWrkMemberRecords as $key) { ?> {
                        id: "<?= $key->member_id ?>",
